...
// 아 정렬하고 싶다 ==> sort
배열.sort()     // 정렬 완료 문자열 기준으로 앞글자로 정렬하게 됨
console.log(배열)

// 숫자 정렬하고 싶으면 a(앞에꺼), b(뒤에꺼) 비교해서
// 음수가 나오면 (앞에꺼가 작으면) a, b로 정렬
// 양수가 나오면 (뒤에꺼가 작으면) b, a로 정렬
배열.sort((a,b)=>a-b)
console.log(배열)
...
// filter : 내가 원하는 값만 남겨라 (return 값에 해당하는 요소만 남김)
// 아이디가 2이상인 객체만 남기자
var 새정렬 = 객체배열.filter((e)=>{
    // return (조건식)  -> 조건식에 부합하는 것만 필터
    return e['id'] >= 2
})
...
// 길이계산을 자동으로 for(let i=0;i<달러;i++){}
달러.forEach((e)=>{
    console.log(e)          // 안에 있느 ㄴ요소들을 하나씩 돌아가면서 반복문 사용
})
